...
def compute_snapshot(
    ...
    if pd.notna(roe):
        score += max(0.0, min(20.0, roe))
    else:
        reasons.append("missing_roe")
    if pd.notna(pe) and pe > 0:
        score += max(0.0, min(20.0, (25.0 - pe)))
    else:
        reasons.append("invalid_pe")
    if pd.notna(pb) and pb > 0:
        score += max(0.0, min(15.0, (4.0 - pb) * 5.0))
    else:
        reasons.append("invalid_pb")
    score += max(0.0, min(15.0, dy))

    if pd.notna(momentum_6m):
        score += max(0.0, min(20.0, momentum_6m * 100.0))
    else:
        reasons.append("missing_momentum")
    if pd.notna(drawdown_1y):
        score += max(0.0, min(10.0, (drawdown_1y + 0.30) * 100.0 / 3.0))
    else:
    ...
    entry_pass = bool(
        ...
        and momentum_6m > 0.0
        and pd.notna(drawdown_1y)
        and drawdown_1y >= -0.30
    )

    reason = "ok" if entry_pass else ",".join(reasons) or "rule_not_met"
